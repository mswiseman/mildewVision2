...
def init_model(model):
    ...
    if model['model_type'] == 'GoogleNet':
        ...
        # IMPORTANT: if feature_extract=True, re-enable gradients for the new head
        if feature_extract:
...
def save_small_rgb(path: Path, rgb01: np.ndarray, fmt="jpg", quality=85, max_side=None):
    """
    rgb01: float32 [0,1] RGB (H,W,3)
    """
    ...
    if max_side is not None:
        ...
        if m > max_side:
            scale = max_side / m
            im = im.resize((int(w * scale), int(h * scale)), resample=Image.Resampling.LANCZOS)
