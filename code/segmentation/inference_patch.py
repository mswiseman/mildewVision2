...
from create_patches import create_patches, recompose
...
if __name__ == "__main__":
    ...
    # the following arguments are loaded from the inference.sh shell script and then populated in this script
    parser = argparse.ArgumentParser()
    parser.add_argument('--model_type', default='DeepLab', help='model used for training')
    parser.add_argument('--weights', action='store_true', help='use pretrained model parameters')
    ...
    parser.add_argument('--cuda_id',  default="0", help='specify cuda')
    parser.add_argument('--img_folder', type=str, default='images', help='image folder identifier')
    parser.add_argument('--dataset_path', type=str, required=True, help='path to data')
    parser.add_argument('--model_path', type=str, required=True, help='path to model')
    parser.add_argument('--HDF5', type=bool, default=False, help='path to model')
    parser.add_argument('--group', type=str, default='baseline', help='exp group')
    parser.add_argument('--set', default='val', choices=['train', 'val', 'test'], help='use train/val/test set for inference')
    parser.add_argument('--thicken', action='store_true', help='thicken hyphal lines')
    parser.add_argument('--seg_idx', type=str, help='num. cv')
    parser.add_argument('--erosion', action='store_true', help='erodes hyphal lines')
    parser.add_argument('--trays', nargs='+', help='tray id, e.g. 1')
 ...
 #   image_folder = dataset_root_path.joinpath(opt.img_folder).joinpath(opt.trays)
    image_folder = dataset_root_path.joinpath(opt.img_folder).joinpath(os.path.join(*opt.trays))


    print("Result root path: ", result_root_path)
    print("Dataset root path: ", dataset_root_path)
    print("Image folder: ", image_folder)
    print("Model para: ", model_para)

    # os.getcwd()).parent moves up one directory
    # os.getcwd()).parent.parent moves up two directory

    output_folder = Path(
        os.getcwd()).parent.parent / 'results' / 'journal' / 'segmentation' / 'segmentation' / 'cross_validation' / f'cv{opt.seg_idx}' / 'figures'
    ...
    # Means and sd from the test dataset
    ...
    default_cmap = LinearSegmentedColormap.from_list(
        'MyColor', ['black', 'white', 'red'])

    # This code initializes a Pandas DataFrame and a color map.
...
for idx in range(images.shape[0]):
    ...
    # Create patches
    patch_size = (224, 224)  # Change patch size as desired
    img_patches = create_patches(cur_img, patch_size)
    mask_patches = create_patches(cur_mask, patch_size)
    pred_patches = []

    # Loop over patches and make inferences
    for i, (img_patch, mask_patch) in enumerate(zip(img_patches, mask_patches)):
        preproc_img = test_transform(img_patch).unsqueeze(0).to(device)

        if opt.thicken:
            kernel = np.ones((5, 5), np.uint8)
            dilated_mask = cv2.dilate(mask_patch, kernel, iterations=1)
            mask_patch = dilated_mask

        preds = model(preproc_img)
        pred_mask_patch = torch.argmax(preds['out'], dim=1).cpu().numpy()[0]

        if opt.erosion:
            kernel = np.ones((5, 5), np.uint8)
            erosion_mask = cv2.erode(pred_mask_patch.astype(np.uint8), kernel, iterations=1)
            pred_mask_patch = erosion_mask
        
        pred_patches.append(pred_mask_patch)

    # Combine patches
    pred_mask = recompose(pred_patches, cur_img.shape[0], cur_img.shape[1], patch_size[0], patch_size[1])
    ...
    if len(cur_mask[cur_mask != 0]):
        ...
        axs[2].imshow(pred_mask, cmap=default_cmap)

        print(f'number of infected pixels in mask: {len(pred_mask[pred_mask!=0])}')
    ...
    # Patch
    for patch_idx in range(len(img_patches)):
        # Mask
        cur_mask = mask_patches[patch_idx]
        mask_clear_pixels = len(cur_mask[cur_mask == 0])
        mask_infected_pixels = len(cur_mask[cur_mask != 0])
        # Pred
        pred_mask = pred_patches[patch_idx]
        ...
        dilated_mask_copy[dilated_mask_copy == 0] = np.array(-1).astype(np.uint8)
        pred_mask_copy[pred_mask_copy == 0] = np.array(-1).astype(np.uint8)
        ...
        classification = 1 if pred_infected_pixels != 0 else 0
        classification_label = 1 if mask_infected_pixels != 0 else 0
        classification_correct_counts += 1 if classification == classification_label else 0
        y_true_patch.append(classification_label)
        y_pred_patch.append(classification)
        ...
        tmp_df = pd.DataFrame([[idx, classification, classification_label, classification ==
                                classification_label, pred_infected_pixels, mask_infected_pixels, iou, dice]], columns=META_COL_NAMES)
        pred_df = pd.concat([pred_df, tmp_df.astype({'status': bool})], ignore_index=True)
