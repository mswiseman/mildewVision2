...
def load_model(model_para):
    ...
    model_fullpath = str(model_para['model_path'] / model_para['model_filename'])
    cuda_id = model_para['cuda_id']
    device = torch.device(f'cuda:{cuda_id}' if cuda_id is not None else 'cpu')

    ckpt_path = model_fullpath.format(model_para['model_type'], model_para['loading_epoch'])
