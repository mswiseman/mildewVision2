...
def make_single_hue_cmap(name, rgb):
    """0 -> transparent/black, 1 -> rgb"""
...
def overlay_two_binary_masks(
        base_rgb_float01, inf_bin, spor_bin, *,
        alpha=0.5,
        inf_color=(0.0, 1.0, 1.0),  # cyan
        spor_color = (1.0, 0.0, 1.0),  # magenta
        overlap_color = (0.85, 0.6, 1.0)  # light violet (key change)
):
    inf_color = np.array(inf_color, dtype=np.float32)
    spor_color = np.array(spor_color, dtype=np.float32)
    overlap_color = np.array(overlap_color, dtype=np.float32)

    inf_bin = inf_bin.astype(bool)
    spor_bin = spor_bin.astype(bool)

    inf_only = inf_bin & (~spor_bin)
    spor_only = spor_bin & (~inf_bin)
    overlap = inf_bin & spor_bin

    out = base_rgb_float01.copy()
    out[inf_only] = (1 - alpha) * out[inf_only] + alpha * inf_color
    out[spor_only] = (1 - alpha) * out[spor_only] + alpha * spor_color
    out[overlap] = (1 - alpha) * out[overlap] + alpha * overlap_color
    return out
