...
axis_label_font_prop = {'fontname': 'Arial',
                        'fontsize': 8, 'weight': 'bold', 'ha': 'left'}
...
dataset_path = Path(opt.dataset_path) / '6-27-2023_5dpi'

tray = '1'
leaf_disk_image_filename = '013-W2021011-489_R1'
patch_filename = '1_013-W2021011-489_R1_image_patch_65.png'
...
saved_filepath = Path(os.getcwd()).parents[0] / 'results' / 'journal' / 'sanity_check' / 'full'
...
fig_width = 7
fig_height = round(fig_width * (nrows / ncols), 2)
...
if opt.model_type == 'VGG':
    new_layer_order = ['Original', 'Classifier', 'Conv13', 'Conv12', 'Conv11', 'Conv10',
                        'Conv9', 'Conv8', 'Conv7', 'Conv6', 'Conv5', 'Conv4', 'Conv3', 'Conv2', 'Conv1']
elif opt.model_type == 'ResNet':
    new_layer_order = ['Original', 'FC', 'Block4',
                       'Block3', 'Block2', 'Block1', 'Conv1']
else:
    new_layer_order = ['Original', 'FC', 'Mixed_7c',
                       'Mixed_7b', 'Mixed_7a', 'Mixed_6e', 'Mixed_6d', 'Mixed_6c',
                       'Mixed_6b', 'Mixed_6a', 'Mixed_5d', 'Mixed_5c', 'Mixed_5b', 
                       'Conv_4a', 'Conv_3b', 'Conv_2b', 'Conv_2a', 'Conv_1a']
...
figsize = (7.5, 5)
fig = plt.figure(figsize=figsize)
gs = gridspec.GridSpec(nrows=2, ncols=2, wspace=0.25, hspace=0.1)
...
ax0.plot(x, rk_df.loc['Gradient-SG', :].values,
         'mo-', lw=1, markersize=2, linestyle='--', label='Gradient-SG')
...
# ax3.set_xticks([])
ax3.set_xticks(x)
ax3.set_xticklabels(new_layer_order, rotation=-45, **axis_label_font_prop)
# ax3.set_xlabel("")
# ax3.set_yticks([])
# ax3.set_ylabel("")
...
# plt.plot(x, rkhog.loc['GradCAM', :].values, 'co-',
#          lw=1, markersize=2, linestyle='--', label='GradCAM')
...
# plt.plot(x, rk_df.loc['DeepLift', :].values,
#          'm^--', lw=1, markersize=2, linestyle='--', label='DeepLift')
# plt.fill_between(x, rk_df.loc['DeepLift', :].values-rk_df2.loc['DeepLift', :].values,
#                  rk_df.loc['DeepLift', :].values +
#                  rk_df2.loc['DeepLift', :].values,
#                  facecolor='y', alpha=0.05)

# plt.plot(x, rk_df.loc['Explanation\nMap', :].values,
#          'ko-', lw=1, markersize=2, linestyle='--', label='Explanation\nMap')
# plt.fill_between(x, rk_df.loc['Explanation\nMap', :].values-rk_df2.loc['Explanation\nMap', :].values,
#                  rk_df.loc['Explanation\nMap', :].values +
#                  rk_df2.loc['Explanation\nMap', :].values,
#                  facecolor='k', alpha=0.05)
...
# elif opt.model_type == 'ResNet':
#     new_layer_order = ['Original', 'FC', 'Layer4', 'Layer3', 'Layer2', 'Layer1', 'Conv1']
# else:
#     new_layer_order = ['Original', 'FC', 'Mixed_7c', 'Mixed_7b', 'Mixed_6e', 'Mixed_5b', 'Conv_1a']
# plt.xticks(x, new_layer_order, rotation=-45, **axis_label_font_prop)

# plt.yticks(**axis_font_prop)

# plt.axhline(y=0.0, color='r', linestyle='--')
# plt.axvline(x=2.0, ymin=0.0, ymax=1.0, color='k', linestyle='--')

# if opt.model_type == 'Inception3':
#     plt.legend(bbox_to_anchor=(1.04,1), borderaxespad=0, fontsize=6, frameon=False)
# # plt.legend(loc='upper right', ncol=1, fontsize=6, frameon=False)
